...
import { getUnitNotifications, markNotificationAsRead, markAllNotificationsAsRead, deleteNotification } from "@/lib/supabase/notification-actions"
...
export function NotificationBell({ unitNumber, className }: NotificationBellProps) {
  ...
  // Get userId from localStorage for mark all as read
  const getUserId = () => {
  ...
  const loadUnreadCount = async () => {
    ...
    try {
      const { getUnreadUnitNotificationCount } = await import("@/lib/supabase/notification-actions")
      const result = await getUnreadUnitNotificationCount(unitNumber)
  ...
  const loadNotifications = async (pageNum: number = 1, append: boolean = false) => {
    ...
    try {
      ...
      const result = await getUnitNotifications(unitNumber, pageNum, 20)
      ...
      if (result.success && result.notifications) {
        ...
        console.log(`[perf] NotificationBell fetch (page ${pageNum}, ${newNotifications.length} items) in ${Math.round(t1 - t0)}ms`)
      } else {
        toast({
      ...
      console.error("[perf] NotificationBell error:", error)
    } finally {
      setIsLoading(false)
    }
  }
        ...
        const channel = supabase
          ...
          .on(
            ...
            (payload: any) => {
              ...
              // Add to list if it's open or we want to keep list fresh
  ...
  const getNotificationIcon = (type: NotificationType) => {
    ...
    if (!IconComponent) {
      // Fallback to Info icon if type is not found
