...
import { importVendors, type VendorItem } from "@/lib/actions/bulk-import-vendors-actions"
import { useProjectContext } from "@/lib/contexts/project-context"

interface VendorsImportDialogProps {
    open: boolean
    onOpenChange: (open: boolean) => void
    onSuccess?: () => void
}

export function VendorsImportDialog({ open, onOpenChange, onSuccess }: VendorsImportDialogProps) {
    const { toast } = useToast()
    const { selectedProjectId } = useProjectContext()
    const [file, setFile] = useState<File | null>(null)
    const [previewData, setPreviewData] = useState<VendorItem[]>([])
            ...
            try {
                ...
                data.forEach((row, index) => {
                    ...
                    if (name) {
                        mappedData.push({
                            name,
                            contact_person: row['Contact Person']?.toString(),
                            phone: row['Phone']?.toString(),
                            email: row['Email']?.toString(),
                            tax_id: row['Tax ID']?.toString(),
    ...
    const handleImport = async () => {
        ...
        setIsUploading(true)
        try {
            const result = await importVendors(previewData, selectedProjectId)

            if (result.success) {
                toast({
                    title: "Import Successful",
                    description: `Successfully imported ${result.imported} vendors.`,
                })
                onOpenChange(false)
                setFile(null)
                setPreviewData([])
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[600px]">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Upload an Excel file with columns: Name, Contact Person, Phone, Email, Tax ID, Address, Notes.
                    </DialogDescription>
                </DialogHeader>
