...
// Helper function to create Thai-compatible text with better mapping
const createThaiCompatibleText = (text: string, language: PDFLanguage) => {
  if (language === "th") {
    // For Thai, we'll use a better approach
    // Convert Thai characters to a more readable format
    return text.replace(/[ก-ฮ]/g, (char) => {
      // Use a more comprehensive and readable mapping
...
// Helper function to add Thai font support with better approach
const addThaiFontSupport = async (doc: jsPDF) => {
  try {
    // Try to use a font that supports Thai characters
    // We'll use a different approach - try to load a Thai font
    
    // Method 1: Try to use a web font that supports Thai
    try {
      // For now, we'll use a simpler approach
      // Set font to helvetica and handle Thai characters
      doc.setFont("helvetica", "normal");
      
      // Override the text method to handle Thai characters better
      ...
      doc.text = function(text: string | string[], x: number, y: number, options?: any) {
        if (typeof text === 'string') {
          ...
          // Also handle Thai vowels and tone marks
      ...
      console.warn("Failed to load Thai font:", fontError);
...
export const generateBillPDFV3 = async (bill: any, unitInfo: any, settings: Settings, language: PDFLanguage = "en") => {
  const doc = new jsPDF();
...
// Updated to accept settings and language
export const generateReceiptPDFV3 = async (payment: any, bill: any, unit: any, settings: Settings, language: PDFLanguage = "en") => {
    const doc = new jsPDF();
