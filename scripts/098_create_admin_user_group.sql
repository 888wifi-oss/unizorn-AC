-- Create new User Group "แอดมิน" with all modules and Staff base role
-- This script will create a comprehensive admin group

SELECT 
  '===== CREATING ADMIN USER GROUP =====' as debug_info;

-- 1. Create the User Group "แอดมิน"
...
SELECT 
  ...
  'กลุ่มผู้ดูแลระบบที่มีสิทธิ์เข้าถึงทุกโมดูลและเมนู' as description,
  r.id as role_id,
  NULL as company_id,
  true as is_active,
  NOW() as created_at,
...
-- 2. Get the created User Group ID
SELECT 
  'Created User Group' as check_type,
  ug.id,
  ug.name,
  ug.display_name,
  ug.description,
...
-- 3. Add all modules with full permissions
...
-- 4. Verify the created User Group and permissions
SELECT 
  'Verification - User Group' as check_type,
  ug.id,
  ug.name,
  ug.display_name,
  ug.description,
