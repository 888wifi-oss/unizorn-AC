-- Migration: Convert Project-Scoped Groups to Global Groups (FINAL FIXED v4)
...
-- Step 7: Update RLS policies (without role column)
DROP POLICY IF EXISTS "Users can view user group permissions" ON user_group_permissions;
DROP POLICY IF EXISTS "Users can manage user_group_permissions" ON user_group_permissions;
...
-- Step 10: Recreate the view with new structure (only existing columns)
