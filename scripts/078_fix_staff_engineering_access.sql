-- แก้ไขปัญหา Role Staff และ Engineering ไม่พบโครงการ
-- Fix Staff and Engineering role project access issues

-- 1. เพิ่ม project_admin role ให้ user ที่มีปัญหา
...
AND u.id IN (
    ...
    WHERE r2.name IN ('staff', 'engineering')
)
AND NOT EXISTS (
...
-- 2. ตรวจสอบผลลัพธ์
SELECT 
    'After Adding Project Admin Roles' as info,
    ur.user_id,
    u.email,
    u.full_name,
    r.name as role_name,
...
-- 3. ตรวจสอบ user group memberships
SELECT 
    'User Group Memberships After Fix' as info,
    u.id,
    u.email,
    u.full_name,
    ugm.user_group_id,
