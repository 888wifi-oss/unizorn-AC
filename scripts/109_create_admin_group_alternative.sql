-- Create admin group with different approach
-- This script will create the group with a different name and configuration

SELECT 
  '===== CREATING ADMIN GROUP WITH DIFFERENT APPROACH =====' as debug_info;
...
-- 2. Create the group with a different name
...
-- 3. Add all 36 modules using INSERT with VALUES
...
SELECT 
  (SELECT id FROM user_groups WHERE name = 'admin_full_access'),
  module_name,
  true,
  true,
  true,
...
WHERE ug.name = 'admin_full_access'
